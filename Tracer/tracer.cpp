...
int main(int argc, char** argv) {
    ...
    for (unsigned int index = 0; index < sources.size(); ++index) {
        ...
        if (item["type"] == "DirectionalPoint") {
            ...
            a = new DirectionalPointSource(item,
            ...
            a = new IsotropicConicalPointSource(item,
                                                dynamic_cast<Solid*>(geometries[insideof]));
            ...
            a = new IsotropicSurfaceSource(item,
                                           dynamic_cast<Solid*>(geometries[insideof]));
    ...
    for (unsigned int i = 0; i < objList.size(); ++i) {
        for (unsigned int j = 0; j < objList.size(); ++j) {
            ...
            vector<Surface*> common =
                findCommonSurfaces(objList[i], objList[j]);
