...
void IrradianceDetector::Irradiance(string& DetectorDataFileName,
	...
	cout << endl;
	cout << "----------- DETECTOR " << DetectorDataFileName << " INFO ------------"
	     << endl;
	...
	ofile2 << "set ztics border in scale 0,0 nomirror norotate offset character 0, 0, 0 autojustify"
	       << endl;
	ofile2 << "set rtics axis in scale 0,0 nomirror norotate offset character 0, 0, 0 autojustify"
	       << endl;
	ofile2  << "set title \"IRRADIANCE [W/M^2]\\n"
	        << "Detected Rays:" << DetectedRays.size() << "(" << TotalNumberOfRays << ")\\n"
	        << "Total Power: " << TotalFlux << " W\\n"
	        << "Peak Irradiance:" << max(max(FluxData)) << " W/M^2\" " << "offset 0,3" <<
	        endl;
	...
	ofile2 << "splot '"     << DetectorDataFileName << "' u (($1-"
	       << (XPix - 1) / 2.0 << ")/"
	       << XPix / RS_width << "):(-($2-"
	       << (YPix - 1.0) / 2.0 << ")/"
	       << YPix / RS_height << "):3 matrix with image"
	       << endl;
	ofile2.close();
}

void IrradianceDetector::IrradianceCrossSectionVertical(int ColNumber,
	...
	for (int j = 0; j <= YPix - 1; j++) {
		ofile << setw(12) << fixed << setprecision(10) << -RS_height / 2.0 + YSTEP / 2.0
		      + j*YSTEP << "     " << FluxData(YPix - 1 - j, ColNumber) << endl;
	}
...
void IrradianceDetector::IrradianceCrossSectionHorizontal(int RowNumber,
        string& CrossSectionDataFileName) {
	ofstream ofile;
	ofile.open(CrossSectionDataFileName.c_str());
	ofile << fixed << setprecision(5);
	...
	for (int j = 0; j <= XPix - 1; j++) {
		ofile << setw(12) << fixed << setprecision(10) << -RS_width / 2.0 + XSTEP / 2.0
		      + j*XSTEP << "     " << FluxData(RowNumber, j) << endl;
	}
