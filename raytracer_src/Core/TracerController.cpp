...
void TracerController::startTracing() {
  ...
  for (std::vector<Source*>::iterator s = _sources.begin(); s != _sources.end();
      ++s) {
    ...
    cout << "Lost fluorescent radiant power in terminations: "
        << _lostPowerFluor << endl;
    cout << "Lost radiant power in Ray Leaking situations: "
        << Solid::lostLeakEnergy << endl;
    cout << "Lost energy together: " << _lostPower + Solid::lostLeakEnergy
        << endl;
...
#ifndef SAVEASCIIRAYS
  f
      << "                <DataArray type=\"Float64\" Name=\"Wavelength\" NumberOfComponents=\"1\" format=\"appended\" offset=\""
...
#ifndef SAVEASCIIRAYS
  f
      << "                <DataArray type=\"UInt64\"  Name=\"connectivity\" format=\"appended\" offset=\""
      << _totalPointNumber * 3 * 8 + 8 + _totalRayNumber * 8 + 8 << "\">\n";
  f << "                </DataArray>\n";
  f
      << "                <DataArray type=\"UInt64\" Name=\"offsets\" format=\"appended\" offset=\""
      << _totalPointNumber * 3 * 8 + 8 + _totalRayNumber * 8 + 8
          + _totalPointNumber * 8 + 8 << "\">\n";
  f << "                </DataArray>\n";
#endif
  f << "            </Lines>\n";
  f << "        </Piece>\n";
