  ...
  if (fromObject == parent) {
    if (_neighbours.size() > 0) {
      ...
      if (coSurf.size() > 0) {
        ...
        for (vector<Surface*>::iterator it = coSurf.begin(); it != coSurf.end();
            ++it) {
        ...
        for (vector<Surface*>::iterator it = coSurf.begin(); it != coSurf.end();
          ...
          if ((*it)->parent->id() > recID
              && arma::dot((*it)->normal(ray.location), ray.direction) < 0) {
            recID = (*it)->parent->id();
            receiverSurf = *it;
          }
        }
...
AbstractGeometry* Surface::receiveRay(Ray& ray, AbstractGeometry* from,
                                      AbstractGeometry* to) {
...
void Surface::addNeighbour(Surface* newNeighbour) {
  ...
  if (find(_neighbours.begin(), _neighbours.end(), newNeighbour)
      == _neighbours.end()) {
    cout << "Set neighbor: " << newNeighbour->objName << " ("
        << newNeighbour->parent->objName << ") for " << objName << " ("
        << parent->objName << ")" << endl;
