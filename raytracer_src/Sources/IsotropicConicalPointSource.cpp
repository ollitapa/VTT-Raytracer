...
IsotropicConicalPointSource::IsotropicConicalPointSource(
    Solid* theEnclosingObject, uint64_t n_rays, double coneHalfAngle) :
    Source(theEnclosingObject, n_rays) {
    _limit_low = cos(coneHalfAngle * M_PI / 180.0);
    direction << 0 << 0 << 1.0;
    location << 0 << 0 << 0;
}

IsotropicConicalPointSource::IsotropicConicalPointSource(Json::Value jsonData,
    ...
    checkRequiredJSONParam(jsonData, "coneHalfAngle", this);
    // Number of rays
    ...
    _limit_low = cos(jsonData["coneHalfAngle"].asDouble() * M_PI / 180.0);
}

IsotropicConicalPointSource::~IsotropicConicalPointSource() {
}

AbstractGeometry* IsotropicConicalPointSource::startingObject() {
    return (enclosingObject);
}

Ray IsotropicConicalPointSource::generateRay() {
    Ray ray;
    _initialiseRay(ray);
    ...
    // generated ray.
    double tht = acos(rndrangeDouble(_limit_low, 1.0));
